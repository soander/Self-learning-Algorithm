...
class Solution:
    def binary_search(self, nums: List[int], target: int) -> int:
        """
        The methods binary search version1 that [left, right]
        ...
        # Use while loop to do binary search, notice that left can equal to right
        while left <= right:
            middle = left + (right - left) // 2
            # Consider three situations
            if nums[middle] < target:
                left = middle + 1
            elif nums[middle] > target:  # right index must minus 1 because of [left, right]
                right = middle - 1
            else:
                return middle
